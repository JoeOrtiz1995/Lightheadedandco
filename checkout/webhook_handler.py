...
class StripeWH_Handler:
  ...
  def _send_confirmation_email(self, order):
    """
    Used to send customers a confirmation email
    """
  ...
  def handle_payment_intent_succeeded(self, event):
    ...
    # Updates empty fields in shipping details to "None"
    for field, value in shipping_details.address.items():
      if value == "":
        shipping_details.address[field] = None

    # Updates profile details if save_info box is checked
    profile = None
    ...
    # print(intent)
    self._send_confirmation_email(order)
    return HttpResponse(content=f'Webhook received: {event["type"]} | SUCCESS: Order creted in Webhook', status=200)
